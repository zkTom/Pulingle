  ...
  <div class="issue" id="issue" v-if="!loading">
    ...
    <div class="clearfix row issue-container">
        ...
        <!-- :class="{'fixed': isFixed}"> -->
          ...
          <div class="personal-info">
            <a href="javascript:void(0);">
              ...
              :src="visitedUserInfo.profilePictureUrl | defaultAvatar('?x-oss-process=image/resize,m_fixed,h_70,w_70')" alt="">
            </a>
            <span class="nickname">{{visitedUserInfo.nickname}}</span>
            <span class="btn-group myself-group" v-if="userInfo.user_id == friendId">
              <span class="num issue-num">
                <strong>{{visitedUserInfo.momentNum}}</strong> <br/>动态</span>
              <span class="border-line"></span>
              <span class="num friend-num">
                <strong> {{visitedUserInfo.friendNum}}</strong> <br/>好友</span>
            </span>
            <span class="btn-group" v-else>
              <button class="btn btn-primary" v-if="visitedUserInfo.identity != 1"  @click="addFriend()">添加好友</button>
              <button class="btn btn-info" v-else @click="sendMessage()">私信</button>
            </span>
      ...
      <div class="col-lg-offset-1 col-md-offset-1 col-sm-offset-1 col-xsoffset-0
        ...
        v-show="isFixed"></div>
      <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 issues">
        <template v-if="page.resultList.length">
          ...
          :page-num="page.pageNum" :current-page="page.currentPage">
            <issue-item v-for="(item, index) in page.resultList" :key="index" 
            :visited-user-info="visitedUserInfo" :item="item"></issue-item>
          </pul-loadmore>
        </template>
      ...
      <div class="col-lg-2 col-md-2 col-sm-2 col-xs-0 new-issues-mask" v-show="isFixed"></div>
    </div>
  </div>
</template>
<script>
...
import {queryMomentsByUserId, queryRecommendMoments} from '../../api/MomentApi'
import {acceptFriendRequest, queryFriendMomentStatus} from '../../api/UserApi'
import {sendFriendRequest} from '../../api/MessageApi'
...
export default {
  ...
  computed:{  
    ...
    signature(){
      let signature = this.visitedUserInfo.signature
  ...
  methods:{
    ...
    async loadmore(){
      ...
      try {
        ...
        let res = await queryMomentsByUserId(params);
        ...
        if (res.status == 1) {
          ...
          this.visitedUserInfo = res.data.userInfo;
          let newPage = Page.as(page)
          this.page.merge(newPage)
        } else {
    ...
    addFriend(){
      let receUserId = this.friendId;
      let sendUserId = this.userInfo.user_id;
      if (!sendUserId) return this.$message.warning('请先登录')
      if (receUserId) {
        let params = {
          sendUserId: sendUserId,
          receUserId: receUserId
        }
        sendFriendRequest(params).then(res => {
          if (res.status == 1) this.$message.success('发送成功')
          else this.$message.warning(res.msg+'，不要重复申请')
        }).catch(error => {
          console.log(error)
          this.$message.warning('发送好友请求失败')
        })
      } else {
        this.$message.warning('发送好友请求失败')
      }
  ...
  watch:{
    ...
    friendId(newVal, oldVal){
      this.$store.commit('set_friendId', newVal)
